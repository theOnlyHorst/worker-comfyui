...
ARG BASE_IMAGE=docker.io/nvidia/cuda:12.8.1-cudnn-runtime-ubuntu24.04
...
RUN if [ "$ENABLE_PYTORCH_UPGRADE" = "true" ]; then \
      /comfyui/.venv/bin/pip install --force-reinstall torch torchvision torchaudio --index-url ${PYTORCH_INDEX_URL}; \
    fi
...
RUN uv pip install runpod requests websocket-client nextcloud_pywebdav
